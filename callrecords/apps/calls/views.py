        ...
        if request.data['type'] == 'start':
            ...
            if serializer.is_valid():
                ...
                if fixed_fee is not None:
                    f_price = fixed_fee.price

                if start.date() == end.date():
                    price = one_day(start, end)
                    bill = Bill.objects.create(price=price['price'] + f_price, fixed_fee=fixed_fee, call_start=result.first(), call_end=end_date)
...
def two_days(start, end):
    ...
    new_s = datetime.combine(
        start.date() + timedelta(days=1), time(hour=0)
    )
    new_e = datetime.combine(
        end.date() - timedelta(days=1), time(hour=23, minute=59, second=59)
    )
    total_days = relativedelta.relativedelta(new_e + timedelta(seconds=1), new_s).days
    result_start = search_periods_in_time(
        start=start,
        end=datetime.combine(start.date(), time(hour=23, minute=59, second=59))
    )
    result_end = search_periods_in_time(
        start=datetime.combine(end.date(), time(hour=0)),
        end=end
    )
...
def search_fixed_fee(s_time):
    ...
    result = FixedFee.objects.raw("select * from bills_fixedfee where %s >= start and %s <= bills_fixedfee.end",
                                  [s_time, s_time])
    try:
        r = result[0]
    except IndexError:
